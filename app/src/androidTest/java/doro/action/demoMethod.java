...
public class demoMethod extends VP4{
...
//            }
    ...
    private static String [] checkName() {//检查照片名字是否正常
        ...
        try {
            switchtoGallery();
            clickIWantToButton();
            if(VideoDetailOption.exists()){
                VideoDetailOption.click();
                PictureName = VideoDetailOption.getText();
            }else if(PictureDetailOption.exists()){
                PictureDetailOption.click();
                PictureName = PictureNameOption.getText();
            }
